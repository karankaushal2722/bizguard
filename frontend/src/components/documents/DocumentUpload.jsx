...
function buildDocPrompt(profile, docType, fileName) {
  ...
  return `You are Amira, a warm AI advisor for BizGuard. The user (${profile.owner_name}, ${profile.business_name}, ${profile.industry}, ${profile.state}) uploaded a ${docTypeLabel} called "${fileName}".

Analyze this document and provide:
1. **What this document is** - plain English explanation
2. **What it means for ${profile.owner_name}** - key facts, amounts, dates, deadlines
3. **Urgency level** - is there a deadline? How urgent?
4. **Recommended next steps** - numbered list of exactly what to do
5. **Questions to ask** - what to gather or clarify

Be warm, specific, and reassuring. Address them by first name. Focus on their industry (${profile.industry}) and state (${profile.state}).
End with: "What questions do you have about this document? I am here to help you through every step."
Add a brief disclaimer that for complex legal/tax matters they should consult a licensed professional.`
}
...
export default function DocumentUpload() {
  ...
  async function analyzeDocument() {
    ...
    try {
      ...
      const messages = [{ role: 'user', content: [
        ...
        { type: 'text', text: `Please analyze this ${DOC_TYPES.find(d => d.id === docType)?.label || 'document'}.` }
      ]}]
      ...
      if (user) supabase.from('document_analyses').insert({ user_id: user.id, file_name: file.name, doc_type: docType, analysis: data.content }).then(() => {}).catch(() => {})
    } catch (err) { setError('Analysis failed. Please try again. ' + err.message) }
  ...
  async function sendFollowUp() {
    ...
    } catch { setConversation(prev => [...prev, { role: 'assistant', content: 'Sorry, something went wrong. Please try again.' }]) }
    ...
    <div className={styles.page}>
      ...
      <div className={styles.layout}>
        <div className={styles.uploadPanel}>
          ...
          <div className={styles.sectionLabel} style={{marginTop:'1rem'}}>Upload your document</div>
          ...
          {!docType && <p className={styles.hint}>Select a document type to get started</p>}
        ...
        <div className={styles.analysisPanel}>
          {!analysis && !loading && (
            <div className={styles.emptyState}>
              ...
              <p>Upload a document and she will explain it in plain English — what it means, what is at stake, and exactly what to do next.</p>
            ...
            <div className={styles.analysisContent}>
              {conversation.slice(1).map((msg, i) => (
                <div key={i} className={`${styles.msg} ${msg.role === 'user' ? styles.msgUser : styles.msgAmira}`}>
                  {msg.role === 'assistant' ? (
                    <>
                      <div className={styles.amiraLabel}><div className={styles.amiraAvatar}><svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M12 2L4 6v6c0 5.25 3.5 10.15 8 11.5C16.5 22.15 20 17.25 20 12V6L12 2z" fill="white"/></svg></div>Amira</div>
                      <div className={styles.msgContent} dangerouslySetInnerHTML={{ __html: msg.content.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br/>') }} />
                    </>
                  ) : <div className={styles.msgContent}>{msg.content}</div>}
                </div>
              ))}
              {followUpLoading && <div className={styles.msg}><div className={styles.typingDots}><span/><span/><span/></div></div>}
              <div className={styles.followUpArea}>
                <div className={styles.followUpRow}>
                  <textarea className={styles.followUpInput} placeholder="Ask a follow-up question about this document..." value={followUp} onChange={e => setFollowUp(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendFollowUp() }}} rows={1} />
                  <button className={styles.followUpBtn} onClick={sendFollowUp} disabled={!followUp.trim() || followUpLoading}>
