LandingPage.jsximport styles from './LandingPage.module.css'
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.section} id="how">
        ...
        <div className={styles.steps}>
          {[
            ...
            ['Meet Amira, your personal advisor', 'Amira knows your business profile and gives answers specific to your situation.'],
            ...
            <div key={i} className={styles.step}>
              ...
              <div className={styles.stepContent}><h3>{title}</h3><p>{desc}</p></div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className={styles.industries}>
        ...
        <div className={styles.pills}>{INDUSTRIES.map((ind, i) => <div key={i} className={styles.pill}>{ind}</div>)}</div>
      </section>
        ...
        <div className={styles.pricingGrid}>
          <div className={styles.pricingCard}>
            ...
            <div className={styles.planFeatures}>
              {['10 questions per day','Basic compliance guidance','All 12 industries','6 languages + voice'].map((f,i) => <div key={i} className={styles.planFeature}><span className={styles.check}>✓</span>{f}</div>)}
            </div>
            <button className={`${styles.planBtn} ${styles.planBtnSecondary}`} onClick={onGetStarted}>Get started free</button>
          </div>
          <div className={`${styles.pricingCard} ${styles.pricingFeatured}`}>
            ...
            <div className={styles.planFeatures}>
              {['Unlimited questions','Full legal guidance','Document analysis','IRS notice decoder','Contract review','Compliance checklists'].map((f,i) => <div key={i} className={styles.planFeature}><span className={styles.check}>✓</span>{f}</div>)}
            </div>
            <button className={`${styles.planBtn} ${styles.planBtnPrimary}`} onClick={onGetStarted}>Upgrade to Business Pro</button>
          </div>
          <div className={`${styles.pricingCard} ${styles.pricingEnterprise}`}>
            ...
            <div className={styles.planFeatures}>
              {['Everything in Business Pro','Multi-user team access','Document history','Custom reports','Dedicated support','API access'].map((f,i) => <div key={i} className={styles.planFeature}><span className={styles.checkEnterprise}>✓</span>{f}</div>)}
            </div>
            <button className={`${styles.planBtn} ${styles.planBtnEnterprise}`} onClick={onGetStarted}>Upgrade to Enterprise</button>
          </div>
        </div>
      </section>
      ...
      <div className={styles.cta}>
        ...
        <button className={styles.ctaBtn} onClick={onGetStarted}>Get started free — no credit card</button>
        ...
        <div className={styles.footerInner}>
          <div className={styles.footerLogo}>
            <div className={styles.navLogoIcon} style={{width:28,height:28,flexShrink:0}}>
              <svg viewBox="0 0 24 24" fill="none" width="14" height="14"><path d="M12 2L4 6v6c0 5.25 3.5 10.15 8 11.5C16.5 22.15 20 17.25 20 12V6L12 2z" fill="white"/></svg>
            </div>
            BizGuard
          </div>
          <p className={styles.footerTagline}>AI-powered legal and compliance protection for small business owners.</p>
          <p className={styles.footerDisclaimer}>© 2025 BizGuard. General guidance only — not a substitute for licensed legal or tax advice.</p>
        </div>
      </footer>
    </div>
  )
